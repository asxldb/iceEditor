...
z.ajax({url:z.uploadUrl,data:formData,success:function(res){if(res){for(var f=0;f<res.length;f++){if(res[f].error){z.filesUpload.error(res[f],res);z.filesUpload.complete(res[f],res);alert(res[f].error);}else{var p=z.c('p');var a=z.c("a");a.href=res[f].url;a.className='download';a.download=res[f].name;a.innerText=res[f].name;a.target='_blank';p.appendChild(a);z.setHTML(p,true);z.filesUpload.success(res[f],res);z.filesUpload.complete(res[f],res);}}
...
if(_z.screenshot){if(clip){var blob=clip.items?(clip.items[0].type.indexOf("image")!==-1?clip.items[0].getAsFile():0):0;if(blob){var reader=new FileReader();reader.onload=function(e){var base64_str=e.target.result;upload(base64_str);}
...
if(_z.pasteText&&t.length==1){_z.setText(text);return;}
if(_z.pasteText||!htmlContent){for(var i=0;i<t.length;i++){if(t[i]){_z.setText('<p>'+t[i].trim()+'<p>',true);}}}else{text=_z.pasteWord(text);text=_z.formatHTML(text);text=_z.pasteHTML(text);text=_z.formatHTML(text);_z.setText(text,true);}
var html=_z.getHTML();_z.d.body.innerHTML=html;html=_z.getHTML();html=html.replace(/<p><\/p>/gi,'').replace(/<\/p><br>/gi,'').replace(/<(span|font|p|b|i|u|s)[^>]*>(<(?!img)[^>]*>)*<\/\1>/gi,'');_z.d.body.innerHTML=html;if(_z.imgAutoUpload){text.replace(/<img .*?src="(.*?)".*?>/gi,function(a,b=''){(function(a,b){_z.ajax({url:_z.uploadUrl,data:{'iceEditor-img':b},success:function(res){if(res&&!res.error){text=text.replace(new RegExp(b,'gi'),res.url);_z.d.body.innerHTML=text;_z.imgUpload.success(res);_z.imgUpload.complete(res);}else{_z.imgUpload.error(res,res);_z.imgUpload.complete(res,res);}},error:function(xhr){_z.imgUpload.error(xhr);_z.imgUpload.complete(xhr);}})})(a,b);});}});function nodePrev(el){if(!el)return false;var node=el.nextSibling;if(node&&node.nodeType!=1)node=nodePrev(node);return node;}
