...
this.disableds=this.c('div');this.disableds.className='iceEditor-disabled';this.editor.appendChild(this.disableds);this.w=this.iframe.contentWindow;this.d=this.iframe.contentDocument;this.d.open();var value=this.value.trim();if(!value.length||value.substr(0,3)!='<p>')value='<p>'+this.value+'<br/></p>';this.d.write('<html><head><style>body{font-family:"Microsoft YaHei";font-size:14px;color:#363636;margin:15px;word-wrap:break-word;word-break:break-all;}img{max-width:100%;}p{margin:0;margin-bottom:5px;}code {color:#696969;background:#eee;padding:2px 5px;display: inline-block;margin:0 5px;border-radius:3px;border:1px solid #dedede;}.iceEditor-code{font-family:Courier New;font-size:13px;line-height:20px;white-space:pre-wrap;}table{margin:10px 0;}table td{padding:8px;line-height:1.42857143;border:1px solid #bdbdbd;}pre{background:#f3f3f3;padding:10px;border-radius:3px;font-family:"Microsoft YaHei";}</style></head><body>'+value+'</body></html>');this.d.close();this.d.body.designMode='On';this.d.body.contentEditable=true;this.d.body.addEventListener('click',function(){_z.parentTagName=_z.range.anchorNode.parentNode.tagName;for(var i=0;i<_z.menu.length;i++){if(_z.d.queryCommandState(_z.menu[i])){_z.menuList[_z.menu[i]].className='iceEditor-actives';}else{_z.menuList[_z.menu[i]].className='';}}})
this.content=this.d.body;if(this.textarea){setInterval(function(){var html=_z.code?_z.html(_z.getHTML()):_z.getHTML();var c=_z.c('p');c.innerHTML=html;var pre=c.getElementsByTagName('pre');for(var s=0;s<pre.length;s++)pre[s].innerHTML=pre[s].innerHTML.replace(/<\/*br>/g,"\n");_z.textarea.innerHTML=_z.unhtml(c.innerHTML);},1000);}
...
z.ajax({url:z.uploadUrl,data:formData,success:function(res){if(res){for(var f=0;f<res.length;f++){if(res[f].error){alert(res[f].error);}else{var p=z.c('p');var a=z.c('img');a.src=res[f].url;p.appendChild(a);z.setHTML(p,true);}}
close.style.display='none';}}})}}});this.createMenu({title:'添加视频',name:'video',icon:'video',id:this.videoId,popup:{width:320,height:170,title:'添加视频',content:'<div class="iceEditor-video"><div><label><input name="video" type="radio" checked value="1"/>自定义</label><label><input name="video" type="radio" value="2"/>B站</label><label><input name="video" type="radio" value="3"/>优酷</label></div><div>URL：<input type="text" name="url" class="iceEditor-videoUrl" placeholder="网络图片地址" value=""/></div><div><label>宽：</label><input type="text" name="width" class="iceEditor-inputWidth" placeholder="px" value=""/><label>高：</label><input type="text" name="height" class="iceEditor-inputHeight" placeholder="px" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div></div>'},success:function(e,z){z.video=z.id(z.videoId);var type;var close=z.video.getElementsByClassName('iceEditor-popup')[0];var url=z.video.getElementsByClassName('iceEditor-videoUrl')[0];var width=z.video.getElementsByClassName('iceEditor-inputWidth')[0];var height=z.video.getElementsByClassName('iceEditor-inputHeight')[0];var btn=z.video.getElementsByClassName('iceEditor-btn')[0];btn.onclick=function(){if(!url.value.length)return alert('视频地址不能为空');var obj=z.video.getElementsByTagName('input');for(var i=0;i<obj.length;i++){if(obj[i].checked)type=Number(obj[i].value);}
if(type===1){var v=z.c('video');v.src=url.value;v.width=width.value.length?width.value:510;v.height=height.value.length?height.value:498;v.controls='controls';}else{var v=z.c('iframe');v.width=width.value.length?width.value:510;v.height=height.value.length?height.value:498;v.setAttribute('frameborder',0);v.setAttribute('allowfullscreen',true);var error='抱歉，无法处理该链接！';if(type===2){var id=url.value.split('?');if(id.length>1){id=id[0].split('video/');if(id.length>1&&id[1].length){v.src='https://player.bilibili.com/player.html?bvid='+id[1];}else{return alert('b站'+error);}}else{return alert('b站'+error);}}else if(type===3){var id=url.value.split('.html');if(id.length>1){id=id[0].split('id_');if(id.length>1&&id[1].length){v.src='https://player.youku.com/embed/'+id[1];}else{return alert('优酷：'+error);}}else{return alert('优酷：'+error);}}}
z.setHTML(v,true);close.style.display='none';z.pd();}}});this.createMenu({title:'最大化',name:'max',icon:'max',data:'max',css:'iceEditor-maxWindow'});this.createMenu({title:'最小化',name:'min',icon:'min',data:'min',css:'iceEditor-minWindow'});this.createMenu({name:'disabled',css:'iceEditor-disabledMask'});};ice.editor.prototype.menuFormat=function(){var _z=this;this.menuHTML();var ul=this.c('ul');ul.className='iceEditor-menu';this.tool.innerHTML='';this.tool.appendChild(ul);for(var i=0;i<this.menu.length;i++){ul.appendChild(this.menuList[this.menu[i]]);if(this.menuList[this.menu[i]].success){this.menuList[this.menu[i]].success(this.menuList[this.menu[i]],_z);}}
...
return false;}}}};ice.editor.prototype.paste=function(){var _z=this;var upload=function(imgBase64){if(!_z.screenshotUpload){var p=_z.c('p');var a=_z.c('img');a.src=imgBase64;p.appendChild(a);_z.setHTML(p,true);return;}
...
var blob=dataURItoBlob(imgBase64);var formData=new FormData();formData.append('file[]',blob);_z.ajax({url:_z.uploadUrl,data:formData,success:function(res){if(res){for(var f=0;f<res.length;f++){if(res[f].error){alert(res[f].error);}else{var p=_z.c('p');var a=_z.c('img');a.src=res[f].url;p.appendChild(a);_z.setHTML(p,true);}}}}})};var getBase64=function(){setTimeout(function(){var imgList=_z.d.body.getElementsByTagName('img');for(var i=0;i<imgList.length;i++){if(imgList[i].src.substr(0,5)=='data:'){upload(imgList[i].src);imgList[i].parentNode.removeChild(imgList[i]);break;}}},10);};this.d.body.addEventListener('paste',function(e){if(!_z.isIE())e.preventDefault();var clip=(window.clipboardData||e.clipboardData||e.originalEvent.clipboardData);var text=clip.getData('Text')||'';if(_z.screenshot){if(clip){var blob=clip.items?(clip.items[0].type.indexOf("image")!==-1?clip.items[0].getAsFile():0):0;if(blob){var reader=new FileReader();reader.onload=function(e){var base64_str=e.target.result;upload(base64_str);}
reader.readAsDataURL(blob);}}
getBase64();}
if(!text.length)return;if(_z.code){_z.setText(text);return;}
var t=text.replace(/[\r|\n]+/g,"\n").split("\n");if(_z.inNodeParent(_z.range.getRangeAt(0).endContainer,'pre')){for(var i=0;i<t.length;i++){t[i]=_z.toText(t[i]);}
_z.setText(t.join('<br>'),true);return;}
if(t.length==1){_z.setText(text);return;}
for(var i=0;i<t.length;i++){if(t[i]){var p=_z.c('p');p.innerHTML=_z.toText(t[i]);_z.setHTML(p,true);}}
var text=_z.getHTML();_z.d.body.innerHTML=text;});function nodePrev(el){if(!el)return false;var node=el.nextSibling;if(node&&node.nodeType!=1)node=nodePrev(node);return node;}
