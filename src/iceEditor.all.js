...
ice.editor = function(id){
	...
	this.ajax.xhr = function(){}; 		//ajax超时时间
	...
	this.d.body.addEventListener('click',function(){
		...
		for(var i=0;i<_z.menu.length;i++){
			if(_z.d.queryCommandState(_z.menu[i])){
				_z.menuList[_z.menu[i]].className = 'iceEditor-actives';
			}else{
				_z.menuList[_z.menu[i]].className = '';
			}
		}
	})
...
ice.editor.prototype={
	...
	ajax:function(json){
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4 && xhr.status === 200) {
				...
				if (xhr.status >= 200 && xhr.status < 300 || xhr.status == 304) {
					...
					_z.ajax.success(res,xhr,json,_z,);
					json.success && json.success(res);
				} else {
					_z.ajax.error(xhr.status,xhr,json,_z);
		...
		function progress(evt) {
			var percent = 0;
			//百分比
			percent = evt.lengthComputable ?　Math.round(oEvent.loaded / oEvent.total * 100) : 0;
			_z.ajax.progress(percent,evt,xhr,_z);
		}
	},
	//创建菜单
...
ice.editor.prototype.menuHTML=function(){
	...
	//隔线
	this.createMenu({title:'隔线',name:'line',css:'line'});

	//插入表情
	var html='<ul class="iceEditor-face"><li><span>表情</span></li>';
	html+='<li class="iceEditor-menuTitle">插入表情</li></ul>';
	this.createMenu({title:'插入表情',name:'face',icon:'face',dropdown:html});
	...
	//添加音乐
	this.createMenu({title:'添加音乐',name:'music',icon:'music',id:this.musicId,popup:{width:320,height:80,title:'添加音乐',content:'<div class="iceEditor-music"><label>音乐链接：</label><input type="text" id="'+this.musicInputId+'" name="url" class="iceEditor-link" placeholder="链接地址" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div>'},
		success:function(e,z){
