...
ice.editor = function(id){
	...
	if(!value.length || value.substr(0,3) != '<p>') value = '<p>'+this.value+'<br/></p>';
...
ice.editor.prototype={
	...
	imagesUpload:function(){
		var _z = this;
		var close = _z.insertImage.getElementsByClassName('iceEditor-popup')[0];
		//输入连接插入图片
		var url = _z.insertImage.getElementsByClassName('iceEditor-insertImageUrl')[0];
		var width = _z.insertImage.getElementsByClassName('iceEditor-inputWidth')[0];
		var height = _z.insertImage.getElementsByClassName('iceEditor-inputHeight')[0];
		var btn = _z.insertImage.getElementsByClassName('iceEditor-btn')[0];
		//绑定输入连接
		...
		//上传图片
		_z.id(_z.imgUploadId).onchange=function(){
			if(!_z.uploadUrl) return alert('请配置uploadUrl项');
	...
	filesUpload:function(){
		...
		_z.id(_z.filesUploadId).onchange=function(){
			if(!_z.uploadUrl) return alert('请配置uploadUrl项');
			console.log(this.files)
			var formData = new FormData();
			for(var i=0;i<this.files.length;i++){
	...
	addVideo:function(){
		var _z = this,type;
		var close = _z.video.getElementsByClassName('iceEditor-popup')[0];
		var url = _z.video.getElementsByClassName('iceEditor-videoUrl')[0];
		var width = _z.video.getElementsByClassName('iceEditor-inputWidth')[0];
		var height = _z.video.getElementsByClassName('iceEditor-inputHeight')[0];
		var btn = _z.video.getElementsByClassName('iceEditor-btn')[0];
		btn.onclick=function(){
			if(!url.value.length) return alert('视频地址不能为空');
			var obj = _z.video.getElementsByTagName('input');
			//获取单选按钮的值
	...
	createMenu:function(json){
		...
		if(json.success)json.success(li,_z);
		//菜单的点击事件
...
ice.editor.prototype.menuHTML=function(){
	//文字大小
	this.createMenu({name:'fontSize',icon:'fontSize',dropdown:'<ul class="iceEditor-fontSize"><li><span class="iceEditor-exec" data="fontSize|1">1</span></li><li><span class="iceEditor-exec" data="fontSize|2">2</span></li><li><span class="iceEditor-exec" data="fontSize|3">3</span></li><li><span class="iceEditor-exec" data="fontSize|4">4</span></li><li><span class="iceEditor-exec" data="fontSize|5">5</span></li><li><span class="iceEditor-exec" data="fontSize|6">6</span></li><li><span class="iceEditor-exec" data="fontSize|7">7</span></li><li class="iceEditor-menuTitle">文字大小</li></ul>'});
	...
	this.createMenu({name:'backColor',icon:'backColor',dropdown:html});
	...
	this.createMenu({name:'foreColor',icon:'foreColor',dropdown:html});

	//加粗
	this.createMenu({name:'bold',data:'bold',icon:'bold'});
	//倾斜
	this.createMenu({name:'italic',data:'italic',icon:'italic'});
	//下划线
	this.createMenu({name:'underline',data:'underline',icon:'underline'});
	//删除线
	this.createMenu({name:'strikeThrough',data:'strikeThrough',icon:'strike'});
	//左对齐
	this.createMenu({name:'justifyLeft',data:'justifyLeft',icon:'alignleft'});
	//居中对齐
	this.createMenu({name:'justifyCenter',data:'justifyCenter',icon:'aligncenter'});
	//右对齐
	this.createMenu({name:'justifyRight',data:'justifyRight',icon:'alignright'});
	//缩进
	this.createMenu({name:'indent',data:'indent',icon:'indent'});
	//取消缩进
	this.createMenu({name:'outdent',data:'outdent',icon:'outdent'});
	//有序列表
	this.createMenu({name:'insertOrderedList',data:'insertOrderedList',icon:'orderedlist'});
	//无序列表
	this.createMenu({name:'insertUnorderedList',data:'insertUnorderedList',icon:'unorderedlist'});
	//下标
	this.createMenu({name:'subscript',data:'subscript',icon:'subscript'});
	//上标
	this.createMenu({name:'superscript',data:'superscript',icon:'superscript'});
	//取消连接
	this.createMenu({name:'unlink',data:'unlink',icon:'unlink'});
	//添加水平线
	this.createMenu({name:'hr',data:'insertHorizontalRule',icon:'min'});
	//清除格式
	this.createMenu({name:'removeFormat',data:'removeFormat',icon:'remove'});
	//全选
	this.createMenu({name:'selectAll',data:'selectAll',icon:'empty'});
	//查看源码
	this.createMenu({name:'code',icon:'code',data:'code'});
	//隔线
	this.createMenu({name:'line',css:'line'});
	//表格
	this.createMenu({name:'table',icon:'table',dropdown:'<ul class="iceEditor-tableMain" id="'+this.tableId+'"><li><div class="iceEditor-tableBox"><div class="iceEditor-tableBgOff"></div><div class="iceEditor-tableBgOn"></div><div class="iceEditor-tableNum">表格：1×1</div></div></li></ul>'});
	//创建连接
	this.createMenu({name:'createLink',icon:'link',id:this.linkId,popup:{width:380,height:110,title:'添加链接',content:'<div class="iceEditor-createLink">创建链接：<input type="text" id="'+this.linkInputId+'" name="url" class="iceEditor-link" placeholder="链接地址" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a><div><label><input type="checkbox" checked="checked" value="1"/> 新窗口打开</label></div></div>'}});
	//音乐
	this.createMenu({name:'music',icon:'music',id:this.musicId,popup:{width:380,height:80,title:'添加音乐',content:'<div class="iceEditor-music">音乐链接：<input type="text" id="'+this.musicInputId+'" name="url" class="iceEditor-link" placeholder="链接地址" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div>'}});
	//附件
	this.createMenu({name:'files',icon:'files',id:this.filesId,popup:{width:450,height:250,title:'附件上传',content:'<div class="iceEditor-insertImage"><input type="file" class="iceEditor-uploadInput" id="'+this.filesUploadId+'" name="file[]" multiple/><div><svg class="iceEditor-icon iceEditor-uploadIcon" aria-hidden="true"><use xlink:href="#icon-files"></use></svg></div><label for="'+this.filesUploadId+'" class="iceEditor-uploadBtn">点击上传附件</label></div>'}});

	//添加图片
	this.createMenu({name:'insertImage',icon:'pic',id:this.imageId,popup:{width:450,height:250,title:'图片上传',content:'<div class="iceEditor-insertImage"><div>URL：<input type="text" name="url" class="iceEditor-insertImageUrl" placeholder="网络图片地址" value=""/>宽：<input type="text" name="width" class="iceEditor-inputWidth" placeholder="宽" value=""/>高：<input type="text" name="height" class="iceEditor-inputHeight" placeholder="高" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div><input type="file" class="iceEditor-uploadInput" id="'+this.imgUploadId+'" name="file[]" accept="image/*" multiple/><div><svg class="iceEditor-icon iceEditor-uploadIcon" aria-hidden="true"><use xlink:href="#icon-pic"></use></svg></div><label for="'+this.imgUploadId+'" class="iceEditor-uploadBtn">点击上传图片</label></div>'}});
	//添加视频
	this.createMenu({name:'video',icon:'video',id:this.videoId,popup:{width:450,height:170,title:'添加视频',content:'<div class="iceEditor-video"><div><label><input name="video" type="radio" checked value="1"/>自定义</label><label><input name="video" type="radio" value="2"/>B站</label><label><input name="video" type="radio" value="3"/>优酷</label></div><div>URL：<input type="text" name="url" class="iceEditor-videoUrl" placeholder="网络图片地址" value=""/></div><div>宽：<input type="text" name="width" class="iceEditor-inputWidth" placeholder="px" value=""/>高：<input type="text" name="height" class="iceEditor-inputHeight" placeholder="px" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div></div>'}});
	//窗口最大化
	this.createMenu({name:'max',icon:'max',data:'max',css:'iceEditor-maxWindow'});
	//窗口最小化
	this.createMenu({name:'min',icon:'min',data:'min',css:'iceEditor-minWindow'});
	//菜单栏禁止
...
ice.editor.prototype.menuFormat=function() {
	...
	//根据菜单配置来初始化菜单功能 ---开始---
	//创建链接
	if(this.inArray('createLink',this.menu)){
		this.link = this.id(this.linkId);
		this.linkInput = this.id(this.linkInputId);
		_z.link.getElementsByClassName('iceEditor-btn')[0].onclick = function(){
			//如果选中的内容存在a标签的话，删除
			var str = _z.getSelectHTML().replace(/<a[^>]+>/ig,'').replace(/<\s*\/a\s*>/ig,'');
			var a = _z.c('a');
			if(_z.link.getElementsByTagName('input')[1].checked) a.target='_blank';
			a.href = _z.linkInput.value;
			a.innerHTML = str;
			_z.setHTML(a,true);
			_z.link.getElementsByClassName('iceEditor-popup')[0].style.display='none';
			_z.pd();
		}
	}
	//添加音乐
	if(this.inArray('music',this.menu)){
		this.music = this.id(this.musicId);
		this.musicInput = this.id(this.musicInputId);
		_z.music.getElementsByClassName('iceEditor-btn')[0].onclick = function(){
			var a = _z.c('audio');
			a.src=_z.musicInput.value;
			a.controls='controls';
			_z.setHTML(a,true);
			_z.music.getElementsByClassName('iceEditor-popup')[0].style.display='none';
			_z.pd();
		}
	}
	if(this.inArray('video',this.menu)){
		this.video = this.id(this.videoId);
		this.addVideo();
	}
	...
	if(this.inArray('table',this.menu)){
		...
		var tableBox = this.table.getElementsByClassName('iceEditor-tableBox')[0];
		var tableBgOn = this.table.getElementsByClassName('iceEditor-tableBgOn')[0];
		var tableNum = this.table.getElementsByClassName('iceEditor-tableNum')[0];
...
ice.editor.prototype.paste=function(){
	...
	this.d.body.addEventListener('keydown', function(e) {
		...
		if (e.ctrlKey || e.metaKey) {
			switch(e.keyCode){
				case 13:{console.log(1);e.preventDefault();break;}
